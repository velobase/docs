...
<Note>
  Amount fields are explicit about their unit: `amount_usd`, `amount_cents`, and `amount_credits`. 1 USD = 100 cents = 1,000,000 credits. A bare `amount` is legacy and means cents. Prefer `amount_usd`.
</Note>
...
Add funds to a customer's wallet. If the customer does not exist yet, Velobase automatically creates them using the `name` and `email` fields you provide.

**`POST /v1/customers/deposit`**

<Note>
  You can also scope the deposit by path: `POST /v1/customers/:customer_id/deposit`.
</Note>

#### Request parameters

<ParamField body="customer_id" type="string" required>
  The unique identifier for this customer in your system. Omit this when using the `:customer_id` path form.
</ParamField>

<ParamField body="amount_usd" type="number" required>
  Amount to deposit, in US dollars. You may instead send `amount_cents` or `amount_credits` (1 USD = 100 cents = 1,000,000 credits). A bare `amount` is legacy and is interpreted as cents.
</ParamField>
