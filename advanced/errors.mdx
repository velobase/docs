---
title: "Understand and handle Velobase API error responses"
description: "Understand the standard Velobase error response format and use the full error codes reference to handle failures programmatically in your application."
---

When a Velobase API request fails, the response includes a structured JSON error object alongside a standard HTTP status code. Every error follows the same format regardless of category, so you can write a single error-handling path in your application.

## Error response format

All errors return an `error` object with three fields:
...
Always branch on `error.code` in your code — `error.message` may change, but `error.code` values are stable across API versions.
...
Check `error.code` first, then fall back to `error.type` for any unrecognized code. Retry `server_error` responses with exponential backoff. Do not retry `insufficient_balance`, `missing_api_key`, or any validation error — these will not resolve on their own.

<Warning>
Do not use `error.message` as a key in your error-handling logic. Message text may be updated to improve clarity. Only `error.code` is guaranteed to remain stable.
</Warning>
