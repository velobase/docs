...
<AccordionGroup>
  <Accordion title="Authentication: 401 Unauthorized">
    ...
    | `api_key_revoked` | The API key has been revoked in the dashboard (API keys section). |
  </Accordion>
  ...
  <Accordion title="Model gateway: customer resolution and routing">
    ...
    | `customer_blocked` | 403 | You blocked this customer in the dashboard (Customers section). Every charge path (gateway calls, freeze, deduct) is refused until you unblock them; deposits still work, and the balance is untouched. |
  ...
  <Accordion title="Payment required: 402">
    ...
    | Code | Description |
    |---|---|
    | `insufficient_balance` | The customer's wallet cannot cover the frozen estimate for this call. The error body includes the required and available credit amounts, so you can drive a paywall or top-up prompt from it. Fix by depositing with `POST /v1/customers/deposit`. |
