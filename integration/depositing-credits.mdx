---
title: "Deposit credits and manage customer wallet categories"
sidebarTitle: "Depositing credits"
description: "Add credits to a customer's balance, organize them into wallet categories, set validity windows, and query the resulting balance breakdown."
---

Use `POST /v1/customers/deposit` to add credits to a customer. Each deposit creates a credit account record that tracks the amount, wallet category, and optional validity window. Customers are created automatically on first deposit, so you never need a separate provisioning step.

## Minimal deposit

The smallest valid request requires only `customer_id` and `amount`.
...
<Note>
  If the customer identified by `customer_id` does not exist, Velobase creates the customer automatically. You do not need to call a separate customer-creation endpoint before depositing.
</Note>

## Add a wallet category

Use `credit_type` to organize credits into named wallet categories. Each category appears separately in the console balance breakdown and can be targeted independently during deduction.
...
`credit_type` accepts any string. Common examples: `default`, `promo_campaign_2026`, `membership_gold`.

## Add a validity period

Use `starts_at` and `expires_at` to restrict when credits are spendable. Credits outside their validity window do not count toward the available balance.
...
Fetch the customer record to see the overall balance and per-wallet-category breakdown.
