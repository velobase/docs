...
Velobase is an OpenAI-compatible AI gateway. You keep your existing OpenAI or
Anthropic SDK and change two things: the **base URL** and one **header** that
says which end-customer to bill. Every call is metered against that customer's
wallet, and the per-call cost comes back on the response headers.

<Note>
**Base URL:** `https://api.velobase.io/v1` — the OpenAI SDK expects the `/v1`
suffix. **Auth:** `Authorization: Bearer <project-key>` (a `vb_live_…` key).
**Who to bill:** the `X-Velobase-Customer: <your-user-id>` header.
</Note>
...
is empty. Control-plane SDKs: pip install velobase-billing / npm i @velobaseai/billing.
````
...
| `x-velobase-cost-credits` | Same cost, in credits (1 cent = 10,000 credits) |
| `x-velobase-balance-credits` | The customer's remaining balance after this call |
...
calls return `402 project_balance_insufficient` regardless of the customer's
balance. Customer deposits are virtual record-keeping and never move real money.
